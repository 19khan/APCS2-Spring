...
private Cell [][] cells = new Cell [this.getRows()][this.getCols()];	
	...
	public void assignValue (String location, String value){		//assigning value to specific cell.
		...
		if (value.indexOf("%") > 0){
			cells[coordinate.getRow()][coordinate.getCol()] = new PercentCell();
		}else if (value.indexOf("(") > 0){
			cells[coordinate.getRow()][coordinate.getCol()] = new FormulaCell();
		}else if (value.indexOf(".") > 0){
			cells[coordinate.getRow()][coordinate.getCol()] = new ValueCell();
		}else{
